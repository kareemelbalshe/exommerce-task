...
export default function ProductSlider({ click , setClick }) {
  ...
  // استخدام useEffect لمراقبة تغييرات click
  useEffect(() => {
    if (click === "left") {
      goPrev();
    } else if (click === "right") {
  ...
  const goNext = () => {
    setCurrentIndex((prevIndex) => 
      (prevIndex + 1) % data.length // الانتقال للصورة التالية مع التأكد من عدم تجاوز العدد
    );
    setClick("")
  };
  ...
  const goPrev = () => {
    setCurrentIndex((prevIndex) => 
      (prevIndex - 1 + data.length) % data.length // الانتقال للصورة السابقة مع الالتفاف إلى النهاية
    );
    setClick("")
  };
  ...
  return (
    <div className="flex gap-5 items-start my-10 overflow-x-scroll hide-scrollbar">
      {data.slice(currentIndex, currentIndex + data.length).map((item, index) => (
        <div className="flex gap-2 items-start flex-col" key={index}>
          <div className="w-60 h-60 flex justify-center items-center bg-gray-300 relative overflow-hidden group">
            ...
            <button className="absolute bottom-[-100%] w-full bg-black text-white p-2 transition-all duration-300 group-hover:bottom-0">
              Add to Cart
            </button>
          </div>
